...
public class UpdateUserProfilePictureCommandHandler(IUserProfileRepository userProfileRepository, IUserRepository userRepository) : IRequestHandler<UpdateUserProfilePictureCommand, Result<byte[]>>
{
    ...
    public async Task<Result<byte[]>> Handle(UpdateUserProfilePictureCommand request, CancellationToken cancellationToken)
    {
        ...
        if (user is null)
        {
            return Result<byte[]>.Invalid($"User with Id {request.UserId} not found");
        }
